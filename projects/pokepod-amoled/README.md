...
网页保持可用。个人设备默认使用固定口令模式；旧版尚未选择口令模式的配置会在首次
启动时迁移到该模式。固定值只在设备屏幕和配网页面显示，运行日志与诊断包不会保存。
配网窗口从用户主动开启时起五分钟内有效；窗口关闭或到期后，口令副本立即从运行内存
清除。高级设置仍可切换到每次启动生成新的 10 位非混淆随机密码；共享设备和公开部署
应选择随机模式。切换到固定模式前必须在配网页面明确确认。更换或清空已保存的腾讯云
密钥时，还必须按设备实体键确认。
...
录音、手机配网和无线麦克风还会写入两级运行诊断。RAM trace 保存最近 64 个细粒度
阶段，跨重启 NVS 环只保存最近 12 个终态、失败、清理和关键生命周期事实，避免在音频
启动和每个阶段同步写 Flash。两级记录都包含关键错误码、内部堆、最大连续块、PSRAM、
运行时间和重启原因；不保存音频、Wi-Fi 密码、腾讯密钥或请求正文。
插回 USB 后可读取：

```sh
./cdc-status.py --command get-runtime-diagnostics
./cdc-status.py --command get-runtime-trace --trace-offset 0 --trace-limit 8
```
...
I2S 超时、最慢读取、预热零读、环形队列高水位和丢帧数。无线会话只有在采集任务已经
产出首个完整 PCM 帧后才通知 Mac 开始，避免 Mac 已按下快捷键却收不到音频。

Link 另有独立活性探针。主循环每次收包、发包都会更新时间；半帧、终态响应或已经
释放业务资源的请求连续五秒没有进展时，固件把请求号、状态、排队帧和卡住类型写入
运行诊断，并只重置 Link 通道。屏幕保持上一帧不再被当成程序活性证据。电脑可读取：

```sh
./cdc-status.py --command link-probe
```

返回值包含当前会话代次、请求号、状态、最近进展时间、恢复次数和上次卡住类型。
`--provisioning-password-mode fixed|random` 使用 USB 专用设置命令，只保存口令策略，
不会触发 Wi-Fi 或腾讯云服务重启。
...
退出配网或 USB/Wi-Fi Link 断开不会取消已经接受的重启意图。配网诊断会记录退出来源，
可以区分触摸返回、BOOT 键、Link、保存完成、五分钟到期和启动超时。

BLE 连接参数采用 Mac/controller 协商的默认值；固件不主动发起连接参数更新。该约束
避开 ESP32-S3 controller 在连接参数更新路径上的中断看门狗风险，同时保留 MTU、认证、
通知队列和会话代次的原有门禁。
