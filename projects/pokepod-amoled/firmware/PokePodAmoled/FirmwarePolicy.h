...
constexpr uint32_t kAudioChunksPerSecond = 1000;
...
constexpr size_t kAudioBytesPerChunk =
    kAudioByteRate / kAudioChunksPerSecond;

static_assert(kAudioBitsPerSample % 8 == 0,
              "PCM bits per sample must be byte aligned");
static_assert(kAudioSampleRate % kAudioChunksPerSecond == 0,
              "audio chunk cadence requires an integral frame count");
...
constexpr TouchAction touchActionAt(int16_t x, int16_t y) {
  ...
  if (y >= 246 && y < 328) return TouchAction::wirelessVoice;
