...
- 同时满足“电量不低于 15% + 非计费 Wi‑Fi”就运行的持久化单任务转写队列；无需打开应用，重启后继续等待。
...
录音先进入 `queued`。设备电量不低于 15% 且 Wi‑Fi 可用时，Android 系统唤醒任务并调用腾讯语音识别；失败会保留原始录音并重新排队。DeepSeek 校对只在 Mac 端点击按钮后调用。
