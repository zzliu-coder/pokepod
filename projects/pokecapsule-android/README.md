...
- 悬浮录音按钮：拖动吸附左右边缘、记忆位置、临时隐藏、彻底关闭、重启恢复。
- 麦克风前台服务：点击即录、每秒倒数、提前停止、60 秒硬停止。
...
- 两条真机录音均完成本地转写并由 Mac 自动校对写回；一条 6.8 秒录音约用 60 秒转写，一条以静音为主的 13 秒录音约用 8 秒。耗时受语音内容影响，不能据此承诺固定速度。
